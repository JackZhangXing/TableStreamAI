        ...
        for chapter_file_path in chapter_file_paths:
            ...
            for item_idx, item in enumerate(chapter_data):
                ...
                if "lensLanguage_end" in item:
                    # todo 将提示词拼接为text和lensLanguage_en组合的
                    text = item["text"]
                    lensLanguage_en = item["lensLanguage_en"]
                    prompt = f"""
                    帮我生成一张二次元美型风格动漫图片，兼具日式动漫与现代化插画特点。内容和描述如下：text：{text}  lensLanguage_en:{lensLanguage_en}
                    """
