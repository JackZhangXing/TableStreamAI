...
"""
该函数用于将图片和音频合成视频，支持图片移动、入场效果及音频变速。若存在同名字幕文件（.srt），会自动添加字幕。主要功能包括：
1.设置视频尺寸（自定义或预设横/竖屏）；
2.调整音频速度并保持音调；
3.图片按指定方向和速度移动，可添加入场动画；
4.自动检测并渲染SRT字幕，支持中文字体与自适应大小；
5.输出为MP4格式视频
"""
                ...
                if subtitles:
                    ...
                    for sub in subtitles:
                        try:
                            ...
                            # 确保字幕时间不超过音频时长
                            start_time = min(start_time, audio_duration)
                            end_time = min(end_time, audio_duration)

                            # 确保字幕持续时间至少为0.5秒，避免过快消失
                            # duration = max(1.0, end_time - start_time)
                            duration =  end_time - start_time



                            # 如果开始时间已经超过了音频时长，则跳过这个字幕
...
# 该函数用于为指定书籍生成视频，主要功能包括：
#
# 1. 从环境变量获取线程数（默认1个）；
# 2. 读取书籍对应章节的JSON文件并排序；
# 3. 统计所有章节中的项目总数以初始化进度条；
# 4. 使用线程池并发处理每个项目，调用[process_item](file://E:\code\TaleStreamAI\app\video.py#L785-L838)函数；
# 5. 处理过程中如有错误则记录日志并返回。
