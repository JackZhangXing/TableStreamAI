...
# 处理单个分镜项的函数
def process_item(item, book_id, json_filename):
    """处理单个分镜项"""
    # 获取id
    item_id = item.get("id", f"index_{item.get('index', 0)}")
        ...
        if os.path.exists(image_path) and not FORCE_RECREATE:
            ...
            while retry_count < 3 and not success:
                try:
                    ...
                    # 保存图片
                    save_result = save_base64_image(base64_image, image_path)
                    del base64_image
                    gc.collect()
...
# 根据ID获取小说文件和提示词列表 - 并发版本
def get_book_content_concurrent(book_id: str):
    # 读取目录下的所有json文件
    ...
    with tqdm(total=total_items, desc="总进度", unit="图") as pbar:
        ...
        for chapter_file_path in chapter_file_paths:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                # 提交任务
                future_to_item = {
                    executor.submit(process_item, item, book_id, json_filename): item
                ...
                # 按原始顺序排序
                updated_items.sort(key=lambda x: int(x["id"]))
            ...
            # 强制清理内存
            del chapter_data, updated_items
            gc.collect()
...
if __name__ == "__main__":
    # 使用并发版本以提高速度
    get_book_content_concurrent("1043294775")
